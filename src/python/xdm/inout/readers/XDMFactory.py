...
def build_device(parsed_netlist_line, reader_state, language_definition):
    ...
    # spectre has ind1='' and ind2=''
    if device_type == 'K':
        for option_tuple in language_definition.admin_dict.get('readOptions'):
            if option_tuple[0] == 'mutualInductorsAsParams' and option_tuple[1] == 'true':
                if not parsed_netlist_line.params_dict.get('ind1') or not parsed_netlist_line.params_dict.get('ind2'):
                    logging.warning("Mutual inductor found, but missing param ind1 or ind2.")
                else:
                    parsed_netlist_line.control_param_list.append(parsed_netlist_line.params_dict.get('ind1'))
                    parsed_netlist_line.control_param_list.append(parsed_netlist_line.params_dict.get('ind2'))
                    del parsed_netlist_line.params_dict['ind1']
                    del parsed_netlist_line.params_dict['ind2']
