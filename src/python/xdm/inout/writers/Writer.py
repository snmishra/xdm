...
class Writer(object):
    ...
    def _write_line(self, ws, xdm_version, from_version, to_version):
        ...
        if isinstance(ws, Device):
            can_convert_special_var, unsupported_var, conflicting_var = handle_special_variables(ws, self._input_admin_writer, self._admin_writer)
            ...
            if self._combine_print_flag and ws.command_type in ['.PRINT', '.PROBE', '.PROBE64'] and can_convert_special_var:
                can_translate_output_var, unsupported_output_vars = handle_output_variables(ws, self._input_admin_writer, self._admin_writer)
                ...
                if ws.command_type in self._output_language_factory.language_definition.unsupported_directive_list:
                    logging.warning("In file:'" + str(os.path.basename(ws.file)) + "' at line:" + str(ws.line_num) + ". Unsupported type: " + ws.command_type + ". Retained (as a comment). Continuing.")
        ...
        if not can_convert_special_var:
            if unsupported_var:
                logging.warning("In file:'" + str(os.path.basename(ws.file)) + "' at line:" + str(ws.line_num) + ". Expression contains unsupported special variable: " + unsupported_var + ". XDM Retained (as a comment). Continuing.")
            ...
            # 2019-10-04 : conflicting variable names now just replace with "XYCE_*", will not be commented out
                ...
                for var in conflicting_var:
                    logging.warning("In file:'" + str(os.path.basename(ws.file)) + "' at line:" + str(ws.line_num) + ". Expression contains variable that conflicts with a special variable in target language: " + var + ". XDM Retained (as a comment). Continuing.")
                    ...
                    for key, param_key in original_keys_to_be_deleted:
                        param_value = ws.props[key][param_key]
                        del ws.props[key][param_key]
                        ws.props[key]["XYCE_"+param_key.upper()] = param_value

        if not can_translate_output_var:
            for unsupported_output_var in unsupported_output_vars:
                logging.warning("In file:'" + str(os.path.basename(ws.file)) + "' at line:" + str(ws.line_num) + ". Expression contains unsupported output variable: " + unsupported_output_var + ". XDM Retained (as a comment). Continuing.")
            ret_str += self._admin_writer['comment'].token_list[0].value[0].value
        ...
        for field in d:
            try:
                ...
                line_count = 0
                if field.label == "outputVariableList" and "XYCE" in to_version.upper() and "*" in r and can_convert_special_var:
                    r_fields = r.split()
                    new_r = ""
                    for ind, r_field in enumerate(r_fields):
                        isExpression = False
                        if (r_field.startswith("'") and r_field.endswith("'")) or (r_field.startswith("{") and r_field.endswith("}")):
                            isExpression = True
                        if "*" in r_field and not "V(*)" in r_field.upper() and not isExpression:
                            logging.warning("Line(s):" + str(self._cur_line+line_count) + ". Unsupported Output Variable in Xyce: " + str(r_field))
                            if "\n" in r_field:
                                new_r += "\n"
                        else:
                            if ind == 0:
                ...
                logging.error('Object at ' + str(ws.line_num[0]) + ' : ' + str(ws) + ' Field: ' + str(field) + ' gave: ' + e.message)
                raise Exception('*****FATAL ERROR*****')
            if 0 < len(r):
        ...
        try:
            ...
            if isinstance(ws, COMMENT):
                ...
                logging.warning("Non-ASCII character detected in the comment within file '" + str(os.path.basename(ws.file)) + "' " + "at line number(s) " + str(ws.line_num) )

            else:
                logging.error("Could not convert non-ASCII character(s) within file '" + str(os.path.basename(ws.file)) + "' " + "at line number(s) " + str(ws.line_num) )
    ...
    def clean_output_variable_list(self, in_list, to_version, line_num):
        ...
        for item in in_list:
            ...
            if "XYCE" in to_version.upper() and "*" in item[0] and (item[0].startswith("'") or item[0].startswith("{")):
                pass
            elif "XYCE" in to_version.upper() and "*" in item[0] and (not "V(*)" in ''.join(item[0].split()).upper()):
                logging.warning("Line(s):" + str(item[1]) + ". Unsupported Output Variable in Xyce: " + str(item[0]))
                continue
            out_list.append(item[0])
        if "XYCE" in to_version.upper() and not out_list:
            logging.warning("Line(s):" + str(line_num) + " Writing .PRINT line with no variables.  Replacing with V(*)")
            out_list.append("V(*)")
        return out_list

    def combine_options(self, to_version):
        lines_to_add = []
        if "XYCE" in to_version.upper() and "HSPICE" in self.input_language.language.upper():
    ...
    def combine_temperatures(self, to_version):
        for aggregate_file in self._temperature_list_aggregate:
            ...
            # If only one temperature present and the output language is Xyce,
            # use special processing for single output .STEP statement
            if "XYCE" in to_version.upper() and len(self._temperature_list_aggregate[aggregate_file]) == 1:
                sweep_obj = SWEEP()
                data_sweep = LIN_SWEEP("TEMP", self._temperature_list_aggregate[aggregate_file][0], self._temperature_list_aggregate[aggregate_file][0], "1")
            ...
            # If output language is Xyce and more than one .TEMP statement present,
            # use special processing for different output (.STEP with a .DATA table)
            elif "XYCE" in to_version.upper() and len(self._temperature_list_aggregate[aggregate_file]) > 1:
                sweep_obj = SWEEP()
            ...
            elif not "XYCE" in to_version.upper():
                directive = Command({}, {}, "GENERATED", [-1], -1)
        ...
        if is_model:
            ...
            for param in obj.params:
                if not model_type.key_params.get(param):
                    logging.info("\n\t\t\t\t\tIn file '" + str(os.path.basename(obj.file)) + "' (" + str(self.input_language.language)
                                 + " " + str(self.input_language.version) + ")" + "\n\t\t\t\t\tAt line number "
                                 + str(obj.line_num) + ", param '" + str(param) + "' does not exist" + "\n\t\t\t\t\tin the target language ("
                                 + str(self.output_language.language) + " " + str(self.output_language.version) + ") for device/level '"
                                 + str(device_type.levelKey) + "'. Continuing and writing param '" + str(param) + "' anyway.")
        else:
            for param in obj.params:
    ...
    def build_output_line(self, obj, writer, output_language_get_directive_by_name):
        ...
        for field in writer:
            try:
                ...
                logging.error('Object ' + str(obj) + ' Field: ' + str(field) + ' gave: ' + e.message)
                raise Exception('*****FATAL ERROR*****')
            if 0 < len(r):
                if r.endswith("\n"):
                    return_string += r
