...
def handle_special_variables(ws, in_admin_writer, out_admin_writer):
    convBool = True
    ...
    if ws.params:
        for key in ws.params:
            item = ws.params[key]
            if item is not None:
                convBool, unsupported_var, conflicting_var, ret_item = token_conversion(item, in_admin_writer['specialvariables'], out_admin_writer['specialvariables'])
                if not convBool:
                    if unsupported_var:
    ...
    for key in ws.props:
        if key == "OUTPUTVARIABLE_LIST":
            ...
            for param_key in ws.props[key]:
                # Dirty hack for "TEMP" handling in Xyce. May remove in future if "TEMP" handling is fixed
                if param_key == "XYCE_TEMP":
                    ws.props[key][param_key] = 'TEMP'
                    continue

                # check param name is not a reserved special variable
                convBool, unsupported_var, conflicting_var, ret_item = token_conversion(param_key, in_admin_writer['specialvariables'], out_admin_writer['specialvariables'])
                if not convBool:
                    if unsupported_var:
                        return convBool, unsupported_var, conflicting_var
                    elif conflicting_var:
...
def handle_output_variables(ws, in_admin_writer, out_admin_writer):
    convBool = True
    unsupported_vars = []
    ...
    for key in ws.props:
        if key == "OUTPUTVARIABLE_LIST":
            for ind,item in enumerate(ws.props[key]):
                convBool, unsupported_var, conflicting_var, ret_item = token_conversion(item, in_admin_writer['outputvariables'], out_admin_writer['outputvariables'])
                if not convBool:
                    if unsupported_var:
...
def token_conversion(item, in_admin_writer, out_admin_writer):
    ...
    for item_field in item_fields:
        ...
        for in_token, out_token in zip(in_admin_writer.token_list, out_admin_writer.token_list):
            ...
            if out_token.ref == "string" and out_token.value and item_field.lower() == out_token.value and in_token.value != out_token.value:
                master_convBool = False
                ...
                converted_item_field = "XYCE_"+out_token.value

            # check if variable is a special variable in source lang
            if in_token.ref == "string" and in_token.value and item_field.lower() == in_token.value:
                ...
                if out_token.value == "temp":
                    converted_item_field = "XYCE_TEMP"
                else:
                    converted_item_field = out_token.value
